...
- Admin login protected by local credentials (passwords stored as SHA-256 hashes)
...
Open the `Package.swift` file inside `swift_form_app` using Xcode 13 or later and run on iOS or macOS Catalyst. The code is organized in the `Sources/SwiftUIFormApp` directory and uses only SwiftUI and the built‑in SQLite3 library, so no external dependencies are required.
