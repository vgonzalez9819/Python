...
- Admin password stored as a SHA‑256 hash for basic security
...
Open the `swift_form_app` package (`Package.swift`) in Xcode 13 or later and run on iOS or macOS Catalyst. The code is organized in the `Sources/SwiftUIFormApp` directory and uses only SwiftUI and the built‑in SQLite3 library, so no external dependencies are required.
