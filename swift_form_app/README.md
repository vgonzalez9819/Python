...
- Admin login protected by a SHA-256 hashed password
...
Open the project in Xcode 16 or later and run on iOS or macOS Catalyst. The code is organized in the `Sources/SwiftUIFormApp` directory and uses only SwiftUI and the built‑in SQLite3 library, so no external dependencies are required.
